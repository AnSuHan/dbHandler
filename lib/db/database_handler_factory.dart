...
class DatabaseHandlerFactory {
  static DatabaseHandler createHandler(ServerModel server, {String? databaseName}) {
    switch (server.type) {
      case 'PostgreSQL':
        return PostgresHandler(server, databaseName: databaseName);
      case 'MySQL':
        return MysqlHandler(server, databaseName: databaseName);
    // 다른 DB 핸들러도 여기에 추가
      default:
